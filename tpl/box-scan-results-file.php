...
else
{
	...
	foreach($acxFileList as $fileName => $v)
	{
        ...
        $cssClass = '';
        if (is_string($sp)){
            $cssClass = (($octP == octdec($sp)) ? 'success' : 'error');
        }
        if (strcasecmp('readme.html', $fileName) == 0)
        {
            if (empty($octP)) {
                // chmod == 000
                continue;
    ...
    echo '<p class="acx-info-box" style="margin: 0 0 7px 0;">';
        echo __('Our suggested permissions are still secure but more permissive in order to not break some servers\' setups.
