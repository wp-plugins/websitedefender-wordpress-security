(function($){
	...
	$(function() {
		var wsdplugin_errorBox = $('.wsdplugin_content .wsdplugin_website_detail .wsdplugin_error_box');
		wsdplugin_errorBox.find('a').click(function() { wsdplugin_errorBox.hide(); return false; });

		$.wsdplugin_showError = function(message, code)
		{
			if (typeof(message) === 'string')
				wsdplugin_errorBox.find('strong').html(message).end().show();
			else if (typeof(message) === 'object' && typeof(message.message) !== 'undefined')
				wsdplugin_errorBox.find('strong').html(message.message).end().show();
		}
...
function wsdplugin_showPassStrength(score)
{
	var ind = jQuery('#pass-strength-result');
	if(score == undefined)
	{
		ind.removeClass()
		return;
...
function wsdplugin_getPassStrength(pass)
{
	if ((typeof pass != 'string') || pass.length == 0 || pass.length < 6) return -1;
	var len = pass.length;

	for(var i=0;i<wsdplugin_badPass.length;i++)
	{
		if(wsdplugin_badPass[i] == pass)
			return 0;
	}
	var score=len*4;
	var a=0;
...
function wsdplugin_updatePassStrength(pass1, username, pass2)
{
	var indicator = jQuery('#pass-strength-result').removeClass();

	if (pass1 == '' && pass2 == '')
	{
		indicator.text('Strength indicator');
		return;
	}

	if ((pass1 != pass2) && pass2.length > 0)
	{
		indicator.text('Mismatch').addClass('short');
		return;
	}

	if ( pass1.toLowerCase() == username.toLowerCase() )
	{
		indicator.text('Bad').addClass('bad');
		return;
	}

	var score = wsdplugin_getPassStrength(pass1);

	if (score == -1)
	{
		indicator.text('Too short').addClass('short');
		return;
	}

	if (score == 0)
	{
		indicator.text('Obvious').addClass('short');
		return;
	}
	...
	if (score < 68)
	{
		indicator.text('Good').addClass('good');
		return;
	}
